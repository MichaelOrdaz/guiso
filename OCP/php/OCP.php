...
class OCP{
  ...
  public function getInfoOrden(){
    ...
    $sql = "SELECT unidad FROM unidad WHERE cliente = '{$info->idCliente}' AND activo = 1";
    $r = $this->db->query($sql);
  ...
  public function getUnidadesCliente( $rtn = false ){//$rtn = return
    ...
    $sql = "SELECT idUnidad, unidad FROM unidad WHERE cliente = '{$cliente}' AND activo = '1' ORDER BY unidad";
    $r = $this->db->query( $sql );
    while( $rows[] = $r->fetch_object() );
    array_pop($rows);
    if($rtn)
